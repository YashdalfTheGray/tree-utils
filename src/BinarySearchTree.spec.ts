...
test('BinarySearchTree', (t: test.Test): void => {
    ...
    t.test('height', (t: test.Test): void => {
        t.test('returns 1 for a single node', (t: test.Test): void => {
            const testTree = new BinarySearchTree<number>((a, b) => a - b, 5);

            t.equal(testTree.height(), 1);
            t.end();
        });

        t.test('returns 2 for a tree with two children', (t: test.Test): void => {
            const testTree = new BinarySearchTree<number>((a, b) => a - b, 5);
            testTree.addChild(3);
            testTree.addChild(7);

            t.equal(testTree.height(), 2);
            t.end();
        });
